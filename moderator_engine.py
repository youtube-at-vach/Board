...
class ModeratorEngine:
    ...
    def decide_next_speaker(self, topic, history, current_round, agents):
        history_text = "\n".join([f"{item['speaker']}: {item['text']}" for item in history])
        available_agents = ", ".join([agent['name'] for agent in agents])

        prompt = f"""
あなたは討論の司会者です。以下の主題と過去の発言を元に、次に発言すべき人物を選び、簡潔な理由を添えてその人物に話を振ってください。また、その人物にどのような質問をすべきか、具体的に提案してください。
...
"""
        ...
        # Extract next speaker name from the entire LLM response, more flexibly
        valid_agent_names = [agent['name'] for agent in agents]
        next_speaker_name = "AI-A" # Default or fallback
        match = re.search(r'(AI-[A-Z])\s*さん', llm_response)
    ...
    def generate_summary(self, topic, history):
        history_text = "\n".join([f"{item['speaker']}: {item['text']}" for item in history])
...
"""
    ...
    def generate_moderator_prompt(self, topic, history, current_round, agents):
        # This function will construct the prompt for the moderator LLM.
        # The actual prompt template from INITIAL.md will be used here.
        history_text = "\n".join([f"{item['speaker']}: {item['text']}" for item in history])
        available_agents = ", ".join([agent['name'] for agent in agents])
